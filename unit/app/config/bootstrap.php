...
$configUnit = array(
    ...
    'pear_path'      => 'C:\Program Files (x86)\Zwamp\vdrive\.sys\php\pear',

    // The directories where the tests reside
    'test_directories' => array(
        "{$root}/../protected/tests/"
    ),
...
set_include_path(
    get_include_path()
    . PATH_SEPARATOR . $root
    . PATH_SEPARATOR . $configUnit['pear_path']
);
...
foreach ( $configUnit['bootstraps'] as $bootstrap ) {
    require $bootstrap;
}

\app\lib\Library::store($configUnit);
